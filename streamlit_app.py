...
st.title("Personality Predictor(Introvert vs Extrovert) By Muhammad Taoheed")
st.markdown("### Please enter the following info:")

time_alone = st.slider("Time Spent Alone(rate:0-10)",0.0,10.0,5.0)
stage_fear = st.radio("Do you have stage fear?",["Yes","No"])
social_event = st.slider("Social Event Attendance(rate:0-10)",0.0,10.0,5.0)
going_outside = st.slider("How often do you go outside?(rate:0-10)",0.0,10.0,5.0)
drained_socializing = st.radio("Do you feel drained after socializing?",["Yes","No"])
friend_circle = st.slider("Size of friends circle(rate:0-10)",0.0,10.0,5.0)
post_frequency = st.slider("How often do you post on social media?(rate:0-10)",0.0,10.0,5.0)
